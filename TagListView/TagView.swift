...
public class TagView: UIButton {
    ...
    var tastiiTagMode = TastiiTagMode.Neutral
    ...
    func changeColorBasedOnTagState() {
        if self.tastiiTagMode == TastiiTagMode.Neutral {
            self.tastiiTagMode = TastiiTagMode.Liked
            UIView.animateWithDuration(0.25, animations: { () -> Void in
                self.backgroundColor = UIColor.greenColor()
                }, completion: nil)
        } else if self.tastiiTagMode == TastiiTagMode.Liked {
            self.tastiiTagMode = TastiiTagMode.Disliked
            UIView.animateWithDuration(0.25, animations: { () -> Void in
                self.backgroundColor = UIColor.redColor()
                }, completion: nil)
        } else if self.tastiiTagMode == TastiiTagMode.Disliked {
            self.tastiiTagMode = TastiiTagMode.Neutral
            UIView.animateWithDuration(0.25, animations: { () -> Void in
